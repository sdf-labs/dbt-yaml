...
use dbt_serde_yaml::{Mapping, Number, Value};
use indoc::indoc;
...
{
    let serialized = dbt_serde_yaml::to_string(&thing).unwrap();
    assert_eq!(yaml, serialized);

    let value = dbt_serde_yaml::to_value(thing).unwrap();
    let serialized = dbt_serde_yaml::to_string(&value).unwrap();
    assert_eq!(yaml, serialized);

    let deserialized: T = dbt_serde_yaml::from_str(yaml).unwrap();
    ...
    let deserialized: T = dbt_serde_yaml::from_value(value).unwrap();
    assert_eq!(*thing, deserialized);

    dbt_serde_yaml::from_str::<serde::de::IgnoredAny>(yaml).unwrap();
}
...
fn test_float() {
    ...
    let float: f64 = dbt_serde_yaml::from_str(indoc! {"
        .nan
    "})
    .unwrap();
    assert!(float.is_nan());
...
fn test_float32() {
    ...
    let single_float: f32 = dbt_serde_yaml::from_str(indoc! {"
        .nan
    "})
    .unwrap();
...
fn test_map_key_value() {
    ...
    let yaml = indoc! {"
        k: v
    "};
    assert_eq!(yaml, dbt_serde_yaml::to_string(&Map).unwrap());
}
