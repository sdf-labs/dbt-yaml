...
use dbt_yaml::value::{Tag, TaggedValue};
use dbt_yaml::{Deserializer, Marker, Span, Value};
use indoc::indoc;
...
{
    let result = dbt_yaml::from_str::<T>(yaml);
...
fn test_serialize_nested_enum() {
    ...
    let error = dbt_yaml::to_string(&e).unwrap_err();
...
fn test_duplicate_keys() {
    fn test_ignore_duplicate_keys(
        ...
        .unwrap();
        assert_eq!(dbt_yaml::to_string(&actual).unwrap(), ignore);
        ...
        .unwrap();
        assert_eq!(dbt_yaml::to_string(&actual).unwrap(), overwrite);
        assert_values_exact!(duplicate_keys, expected_duplicate_keys);
    }
