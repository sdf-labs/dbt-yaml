...
use dbt_yaml::{Mapping, Number, Value};
use indoc::indoc;
...
{
    let serialized = dbt_yaml::to_string(&thing).unwrap();
    assert_eq!(yaml, serialized);

    let value = dbt_yaml::to_value(thing).unwrap();
...
fn test_float() {
    ...
    let float: f64 = dbt_yaml::from_str(indoc! {"
        .nan
    "})
    .unwrap();
    assert!(float.is_nan());
...
fn test_float32() {
    ...
    let single_float: f32 = dbt_yaml::from_str(indoc! {"
        .nan
    "})
    .unwrap();
...
fn test_char() {
    ...
    let yaml = indoc! {"
        '.'
    "};
    assert_eq!(yaml, dbt_yaml::to_string(&ch).unwrap());
...
fn test_map_key_value() {
    ...
    let yaml = indoc! {"
        k: v
    "};
    assert_eq!(yaml, dbt_yaml::to_string(&Map).unwrap());
}
