...
/// fn main() -> Result<()> {
///     let input = "k: 107\n";
///     let de = dbt_yaml::Deserializer::from_str(input);
...
///     let input = "---\nk: 107\n...\n---\nj: 106\n";
///
///     for document in dbt_yaml::Deserializer::from_str(input) {
    ...
    V: Visitor<'de>,
{
    ...
    #[cfg(feature = "yaml_11")]
    if spanned::resolve_timestamps() && is_yaml_11_timestamp(v) {
        return visit_timestamp(visitor, v, repr, style);
    }
...
/// Returns whether `scalar` has the shape of a YAML 1.1 core schema
/// timestamp: either a bare date (`YYYY-MM-DD`) or a full timestamp
/// (`YYYY-M-D(T|t| +)H:MM:SS(.fraction)?( ?(Z|±H(:MM)?))?`).
///
/// This only checks shape; it does not validate that the numbers form a
/// real calendar date or time.
#[cfg(feature = "yaml_11")]
fn is_yaml_11_timestamp(scalar: &str) -> bool {
    fn take_digits(bytes: &[u8], i: &mut usize, min: usize, max: usize) -> bool {
        let start = *i;
    ...
    fn expect(bytes: &[u8], i: &mut usize, b: u8) -> bool {
    ...
    // Bare date form: exactly YYYY-MM-DD, nothing else.
    if bytes.len() == 10
    ...
    // Full timestamp form.
    let mut i = 0;
    if !take_digits(bytes, &mut i, 4, 4) || !expect(bytes, &mut i, b'-') {
        return false;
    }
    if !take_digits(bytes, &mut i, 1, 2) || !expect(bytes, &mut i, b'-') {
        return false;
    }
    ...
    if !take_digits(bytes, &mut i, 2, 2) || !expect(bytes, &mut i, b':') {
        return false;
    }
...
/// Feeds `v`, known to have timestamp shape, to `visitor` as
/// `Value::Tagged(Tag::new("timestamp"), Value::String(v))` by way of
/// `Visitor::visit_enum`. There is no dedicated timestamp variant on the
/// `Visitor` trait, so the tag is synthesized the same way explicit YAML
/// `!tag` scalars already are (see `EnumAccess` above).
///
/// Visitors that don't implement `visit_enum` (most hand-written or derived
/// visitors other than `Value`'s) fail this with a type error rather than
/// falling back to a plain string. That's why timestamp resolution is
/// opt-in: enabling it changes behavior only for callers built to expect a
/// possible `Tagged` timestamp, such as generic `Value` trees.
#[cfg(feature = "yaml_11")]
fn visit_timestamp<'de, V>(
    visitor: V,
    v: &str,
    repr: Option<&'de [u8]>,
    style: ScalarStyle,
...
where
    V: Visitor<'de>,
{
    let content = match parse_borrowed_str(v, repr, style) {
        Some(borrowed) => TimestampContent::Borrowed(borrowed),
        None => TimestampContent::Owned(v.to_owned()),
    };
    visitor.visit_enum(TimestampEnumAccess { content })
}

#[cfg(feature = "yaml_11")]
enum TimestampContent<'de> {
    Borrowed(&'de str),
    Owned(String),
}

#[cfg(feature = "yaml_11")]
struct TimestampEnumAccess<'de> {
    content: TimestampContent<'de>,
}

#[cfg(feature = "yaml_11")]
impl<'de> de::EnumAccess<'de> for TimestampEnumAccess<'de> {
    type Error = Error;
    type Variant = TimestampVariantAccess<'de>;

    fn variant_seed<S>(self, seed: S) -> Result<(S::Value, Self::Variant)>
    where
        S: DeserializeSeed<'de>,
    {
        let tag = StrDeserializer::<Error>::new("timestamp");
        let variant = seed.deserialize(tag)?;
        Ok((
            variant,
            TimestampVariantAccess {
...
#[cfg(feature = "yaml_11")]
struct TimestampVariantAccess<'de> {
    content: TimestampContent<'de>,
}

#[cfg(feature = "yaml_11")]
impl<'de> de::VariantAccess<'de> for TimestampVariantAccess<'de> {
    type Error = Error;
    ...
    {
        match self.content {
            TimestampContent::Borrowed(s) => {
                seed.deserialize(BorrowedStrDeserializer::<Error>::new(s))
            }
            TimestampContent::Owned(s) => seed.deserialize(StringDeserializer::<Error>::new(s)),
        }
    }
