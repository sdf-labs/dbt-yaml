...
/// A type that can be used to index into a `dbt_yaml::Value`. See the `get`
    ...
    I: Index,
{
    ...
    /// Index into a `dbt_yaml::Value` using the syntax `value[0]` or
    /// `value["k"]`.
    ///
    ...
    /// let data: dbt_yaml::Value = dbt_yaml::from_str(r#"{ x: { y: [z, zz] } }"#)?;
    ///
    /// assert_eq!(data["x"]["y"], dbt_yaml::from_str::<Value>(r#"["z", "zz"]"#).unwrap());
    /// assert_eq!(data["x"]["y"][0], dbt_yaml::from_str::<Value>(r#""z""#).unwrap());
    ///
    /// assert_eq!(data["a"], dbt_yaml::from_str::<Value>(r#"null"#).unwrap()); // returns null for undefined values
    /// assert_eq!(data["a"]["b"], dbt_yaml::from_str::<Value>(r#"null"#).unwrap()); // does not panic
    /// # Ok(())
    /// # }
    /// ```
...
where
    I: Index,
{
    /// Write into a `dbt_yaml::Value` using the syntax `value[0] = ...` or
    /// `value["k"] = ...`.
    ///
    ...
    /// ```
    /// # fn main() -> dbt_yaml::Result<()> {
    /// let mut data: dbt_yaml::Value = dbt_yaml::from_str(r#"{x: 0}"#)?;
    ///
    /// // replace an existing key
    /// data["x"] = dbt_yaml::from_str(r#"1"#)?;
    ///
    /// // insert a new key
    /// data["y"] = dbt_yaml::from_str(r#"[false, false, false]"#)?;
    ///
    /// // replace a value in a sequence
    /// data["y"][0] = dbt_yaml::from_str(r#"true"#)?;
    ///
    /// // inserted a deeply nested key
    /// data["a"]["b"]["c"]["d"] = dbt_yaml::from_str(r#"true"#)?;
    ///
    /// println!("{:?}", data);
    /// # Ok(())
    /// # }
