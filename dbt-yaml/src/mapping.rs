...
/// A YAML mapping in which the keys and values are both `dbt_yaml::Value`.
...
/// A type that can be used to index into a `dbt_yaml::Mapping`. See the
...
/// Iterator over `&dbt_yaml::Mapping`.
pub struct Iter<'a> {
...
/// Iterator over `&mut dbt_yaml::Mapping`.
...
/// Iterator over `dbt_yaml::Mapping` by value.
pub struct IntoIter {
...
/// Iterator of the keys of a `&dbt_yaml::Mapping`.
pub struct Keys<'a> {
...
/// Iterator of the values of a `&dbt_yaml::Mapping`.
pub struct Values<'a> {
...
/// Iterator of the values of a `&mut dbt_yaml::Mapping`.
